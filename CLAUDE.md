...
- `order_timestamp()` — the git commit time a content file was first added (fallback: filesystem mtime, for a not-yet-committed file). Breaks ties between same-day posts so they keep their real publishing order. Deliberately *not* plain filesystem mtime: that resets on every fresh checkout/clone (e.g. a cloud session starting from a clean copy), which previously reordered same-day posts at random depending on which session last regenerated the site.
- `build()` — orchestrates: parse all posts, sort by `(data, order_ts)` desc, wipe+regenerate `post/`, write `index.html`, call `build_about()`
